...
export default class MAMNewFileView {
  ...
  setup(){
    document.getElementById('vipera-tools-mam-newfile-path-textfield').value = atom.project.getPaths()[0] + "/NewAPI.mam";
  }

  onCreateButton(callback){
    document.getElementById('vipera-tools-mam-newfile-create').addEventListener('click', function(evt){
      callback(document.getElementById('vipera-tools-mam-newfile-path-textfield').value, evt);
    });
  }
