...
export default {
  ...
  validateCurrentSelectedFile(){
    if (this.currentFileIsMAM){
      //console.log("Current text changed: " + this.currentTextEditor.getLineCount());
      try {
  ...
  subscribeActions() {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'dynamicengine-mam-tools:toggle': () => this.showMAMPane()
    }));
  },
  ...
  generateDoc(jsonSchema){
    ...
    if (!that.docTemplate){
      ...
      var TemplatesView = require("./dynamicengine-mam-doc-template.js");
  ...
  noSchemaFileSelected(){
      this.showElement('vipera-tools-mam-schema-no-mam-file-selected', true);
      this.showElement('vipera-tools-mam-schema-validation-ko', false);
      this.showElement('vipera-tools-mam-schema-validation-ok', false);
  },
  ...
  showMAMPane(){
    ...
    var TemplatesView = require("./dynamicengine-mam-mainview.js");
