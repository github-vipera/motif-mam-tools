describe "MAM grammar", ->
  grammar = null

  beforeEach ->
    waitsForPromise ->
      atom.packages.activatePackage('motif-mam-tools')

    runs ->
      grammar = atom.grammars.grammarForScopeName('source.mam')

  it "parses the grammar", ->
    expect(grammar).toBeDefined()
    expect(grammar.scopeName).toBe 'source.mam'
