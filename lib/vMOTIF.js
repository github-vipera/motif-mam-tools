...
export default class vMOTIF {
  ...
  start(){
    ...
    expressApp.post('/json', function(request,res){
      ...
      try {
        ...
        var header = request.body.req.header;
        var uri = "/" + dom + "/" + app + "/" + srv + "/" + op;
        if (that.jsonSchemas[uri]){
          var requestSchema = that.jsonSchemas[uri];

           var mamRequestSchema =  require("./schemas/motif.request.schema.json");
          //validate the request
          var Ajv = require('ajv');
          var ajv = new Ajv(); // options can be passed, e.g. {allErrors: true}
          allowUnsafeEval(() => allowUnsafeNewFunction(() => ajv.addSchema(mamRequestSchema, mamRequestSchema.id)));
          allowUnsafeEval(() => allowUnsafeNewFunction(() => ajv.addSchema(requestSchema, "__temp")));
          var valid = allowUnsafeEval(() => allowUnsafeNewFunction(() => ajv.validate('__temp', header)));
          if (valid){
            if (requestSchema.response && requestSchema.response.example){
              var jsonResp = {
                  "dom" : dom,
                  "app" : app,
                  "srv" : srv,
                  "op" : op,
                  "header" : requestSchema.response.example
              };
              res.json(jsonResp);
            } else {
              res.status(500).send('Operation ' + uri +' response example not defined');
            }
          } else {
            res.status(500).send('Invalid request (schema validation error): ' + ajv.errorsText());
          }
          ajv.removeSchema("__temp");
          ajv.removeSchema(mamRequestSchema.id);
          ajv = undefined;
        } else {
    ...
    http.listen(port, function(){
      ...
      atom.notifications.addInfo("MAM Mock service started.");
    });
  }
  ...
  stop(){
    ...
    atom.notifications.addInfo("MAM Mock service stopped.");
  }
